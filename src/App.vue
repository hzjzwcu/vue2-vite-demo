<template>
  <div id="app">
    ...
    <p>
      此示例展示了一个内嵌
      <strong>{{ nodeCount.toLocaleString() }}</strong> 个节点 (7层) 树的 Select
      组件。
    </p>
    <p>
      它使用
      <code>vue-virtual-scroll-list</code>
      实现虚拟滚动以保持高性能，并支持多选和模糊搜索。
    </p>
    <div class="controls-wrapper">
      <el-button @click="onClick">点击改选中</el-button>
...
export default {
  ...
  data() {
    return {
      ...
      nodeCount: 0,
      linkage: true, // 控制联动模式
      selectedTreeNodes: ["0-1-1-1", "0-1-1-2"], // 用于 v-model 双向绑定的选中节点 ID 数组
      // 定义树形组件如何解析我们的数据
      treeProps: {
        id: "instId",
        label: "instAttrName",
  ...
  methods: {
    onClick() {
      this.selectedTreeNodes = ["0-1-1-2"];
    },
  },
};
</script>
