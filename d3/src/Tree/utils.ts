...
export function filterNotVisibleLink(this: Tree, d3Selection: LinkSelection): LinkSelection {
  if (this.option.virtual) {
    ...
    const svgWidth = this.svgSize.width;
    const svgHeight = this.svgSize.height;
    const left = -svgWidth / 2;
    const right = svgWidth / 2;
    const top = -svgHeight / 2;
    const bottom = svgHeight / 2;

    const validPoint = (x: number, y: number) => {
      return x >= left && x <= right && y >= top && y <= bottom;
    };
    d3Selection.filter(it => {
      const [x1, y1] = [it.source.x * k, it.source.y * k];
      const [x2, y2] = [it.target.x * k, it.target.y * k];
      return validPoint(x1, y1) || validPoint(x2, y2);
    });
  }
